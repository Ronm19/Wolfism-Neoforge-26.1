...
/** Keeps idle wild adult Water Wolves moving as a loose pack. */
public final class WaterPackCohesionGoal extends Goal {
    ...
    private WaterWolf packmate;
    private int repathDelay;

    public WaterPackCohesionGoal(WaterWolf wolf) {
        this.wolf = wolf;
    ...
    public boolean canUse() {
        ...
        Optional<WaterWolf> rememberedPackmate = this.wolf.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_WATER_PACKMATE.get());
        if (rememberedPackmate.isEmpty()) {
            return false;
        }

        WaterWolf candidate = rememberedPackmate.get();
    ...
    public void tick() {
        if (this.packmate != null && --this.repathDelay <= 0) {
            this.repathDelay = 10;
            this.wolf.moveToWaterAware(this.packmate, SPEED);
        }
    }
