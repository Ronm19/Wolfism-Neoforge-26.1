...
/** Infernal pups retreat from a remembered pack threat instead of fighting. */
public final class InfernalPupRetreatGoal extends Goal {
    ...
    public InfernalPupRetreatGoal(InfernalWolf wolf) {
        this.wolf = wolf;
    ...
    private void chooseEscapePath() {
        ...
        InfernalWolf adult = this.wolf.getBrain()
        ...
        LivingEntity anchor =
                adult != null && adult.isAlive()
                        ? adult
                        : owner;

        if (anchor != null
                && anchor.isAlive()
                && anchor.distanceToSqr(this.threat)
                        > this.wolf.distanceToSqr(this.threat)) {
        ...
        Vec3 away =
                this.wolf.position().subtract(this.threat.position());
        ...
        if (horizontal.lengthSqr() <= 1.0E-6D) {
            double angle =
                    this.wolf.getRandom().nextDouble()
                            * Math.PI * 2.0D;
        ...
        Vec3 destination =
                this.wolf.position().add(horizontal.scale(8.0D));
