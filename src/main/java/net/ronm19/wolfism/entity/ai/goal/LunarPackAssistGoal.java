...
/**
 * Shares an active Lunar Wolf pack threat without overriding a real current fight.
 */
public final class LunarPackAssistGoal extends Goal {
    private final LunarWolf wolf;
    private LivingEntity sharedThreat;

    public LunarPackAssistGoal(LunarWolf wolf) {
        this.wolf = wolf;
    ...
    public boolean canUse() {
        ...
        Optional<LivingEntity> rememberedThreat = this.wolf.getBrain()
                .getMemory(ModMemoryModuleTypes.LUNAR_PACK_THREAT.get());
        if (rememberedThreat.isEmpty()) {
            return false;
        }

        LivingEntity threat = rememberedThreat.get();
        if (!this.wolf.isValidLunarCombatTarget(threat)) {
            return false;
        }
