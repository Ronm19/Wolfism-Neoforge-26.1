...
public final class StormPupFollowAdultGoal extends Goal {
    ...
    private StormWolf adult;
    private int repathDelay;

    public StormPupFollowAdultGoal(StormWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        Optional<StormWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_STORM_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        StormWolf candidate = rememberedAdult.get();
