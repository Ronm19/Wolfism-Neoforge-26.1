...
/** Lightweight social play for Sand Wolf pups while the pack is safe. */
public final class SandPupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.08D;

    private final SandWolf pup;
    private SandWolf playmate;
    private int playTicks;

    public SandPupPlayGoal(SandWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        List<SandWolf> pups = this.pup.level().getEntitiesOfClass(
                SandWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isSandPackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
