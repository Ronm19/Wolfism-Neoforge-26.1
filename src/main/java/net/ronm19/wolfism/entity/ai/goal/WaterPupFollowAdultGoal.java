...
public final class WaterPupFollowAdultGoal extends Goal {
    ...
    private WaterWolf adult;
    private int repathDelay;

    public WaterPupFollowAdultGoal(WaterWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        Optional<WaterWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_WATER_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        WaterWolf candidate = rememberedAdult.get();
    ...
    public void tick() {
        if (this.adult != null && --this.repathDelay <= 0) {
            this.repathDelay = 10;
            this.pup.moveToWaterAware(this.adult, SPEED);
        }
    }
