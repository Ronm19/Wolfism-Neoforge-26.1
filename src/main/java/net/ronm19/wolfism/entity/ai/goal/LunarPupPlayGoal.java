...
/** Lightweight social play so Lunar pups behave like pups when the pack is safe. */
public final class LunarPupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.05D;

    private final LunarWolf pup;
    private LunarWolf playmate;
    private int playTicks;

    public LunarPupPlayGoal(LunarWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        List<LunarWolf> pups = this.pup.level().getEntitiesOfClass(
                LunarWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isLunarPackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
