...
public final class EarthPupFollowAdultGoal extends Goal {
    ...
    private final EarthWolf pup;
    private EarthWolf adult;
    private int repathDelay;

    public EarthPupFollowAdultGoal(EarthWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        if (!this.pup.isBaby()
                ...
                || this.pup.getBrain().hasMemoryValue(ModMemoryModuleTypes.EARTH_PACK_THREAT.get())) {
            return false;
        }

        Optional<EarthWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_EARTH_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        EarthWolf candidate = rememberedAdult.get();
