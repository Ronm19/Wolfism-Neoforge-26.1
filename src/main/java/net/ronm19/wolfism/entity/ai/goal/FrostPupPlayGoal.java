...
/** Lightweight social play so Frost pups behave like pups when the pack is safe. */
public final class FrostPupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.05D;

    private final FrostWolf pup;
    private FrostWolf playmate;
    private int playTicks;

    public FrostPupPlayGoal(FrostWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        List<FrostWolf> pups = this.pup.level().getEntitiesOfClass(
                FrostWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isFrostPackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
