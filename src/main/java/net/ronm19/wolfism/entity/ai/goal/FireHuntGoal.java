...
/**
 * Starts and joins coordinated wild Fire hunts.
 *
 ...
 */
public final class FireHuntGoal extends Goal {
    private final FireWolf wolf;
    private LivingEntity prey;

    public FireHuntGoal(FireWolf wolf) {
        this.wolf = wolf;
    ...
    public boolean canUse() {
        if (this.wolf.isTame()
                ...
                || this.wolf.getBrain().hasMemoryValue(ModMemoryModuleTypes.FIRE_PACK_THREAT.get())) {
            return false;
        }

        Optional<LivingEntity> sharedHunt = this.wolf.getBrain()
                .getMemory(ModMemoryModuleTypes.FIRE_HUNT_TARGET.get());
        if (sharedHunt.isPresent() && isEligible(sharedHunt.get())) {
            this.prey = sharedHunt.get();
            return true;
        }

        Optional<FireWolf> leader = this.wolf.getBrain()
                .getMemory(ModMemoryModuleTypes.FIRE_PACK_LEADER.get());
        if (leader.isPresent() && leader.get() != this.wolf) {
            return false;
        }

        Optional<LivingEntity> nearestPrey = this.wolf.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_FIRE_PREY.get());
    ...
    private boolean isEligible(LivingEntity target) {
        return target.isAlive()
                && FireWolf.isPreferredPrey(target)
                && this.wolf.hasEnoughHuntersFor(target)
                && this.wolf.isValidFireCombatTarget(target);
    }

    @Override
    public void start() {
        if (this.prey != null) {
            this.wolf.getBrain().setMemory(ModMemoryModuleTypes.FIRE_HUNT_TARGET.get(), this.prey);
