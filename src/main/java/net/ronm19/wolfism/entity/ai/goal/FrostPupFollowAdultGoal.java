...
public final class FrostPupFollowAdultGoal extends Goal {
    ...
    private final FrostWolf pup;
    private FrostWolf adult;
    private int repathDelay;

    public FrostPupFollowAdultGoal(FrostWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        if (!this.pup.isBaby()
                ...
                || this.pup.getBrain().hasMemoryValue(ModMemoryModuleTypes.FROST_PACK_THREAT.get())) {
            return false;
        }

        Optional<FrostWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_FROST_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        FrostWolf candidate = rememberedAdult.get();
