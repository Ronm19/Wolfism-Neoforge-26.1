...
/** Lightweight social play so Arctic pups behave like pups when the pack is safe. */
public final class ArcticPupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.05D;

    private final ArcticWolf pup;
    private ArcticWolf playmate;
    private int playTicks;

    public ArcticPupPlayGoal(ArcticWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        if (!this.pup.isBaby()
                ...
                || this.pup.getBrain().hasMemoryValue(ModMemoryModuleTypes.ARCTIC_PACK_THREAT.get())
                || this.pup.getRandom().nextInt(90) != 0) {
            return false;
        }

        List<ArcticWolf> pups = this.pup.level().getEntitiesOfClass(
                ArcticWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isArcticPackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
