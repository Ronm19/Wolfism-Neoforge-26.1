...
public final class TimberPupFollowAdultGoal extends Goal {
    ...
    private TimberWolf adult;
    private int repathDelay;

    public TimberPupFollowAdultGoal(TimberWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        Optional<TimberWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_TIMBER_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        TimberWolf candidate = rememberedAdult.get();
