...
/** Lightweight social play so Fire pups behave like pups when the pack is safe. */
public final class FirePupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.05D;

    private final FireWolf pup;
    private FireWolf playmate;
    private int playTicks;

    public FirePupPlayGoal(FireWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        List<FireWolf> pups = this.pup.level().getEntitiesOfClass(
                FireWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isFirePackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
