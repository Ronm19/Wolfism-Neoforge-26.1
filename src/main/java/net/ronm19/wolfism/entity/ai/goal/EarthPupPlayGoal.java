...
/** Lightweight social play so Earth pups behave like pups when the pack is safe. */
public final class EarthPupPlayGoal extends Goal {
    private static final double PLAY_RADIUS = 10.0D;
    private static final double SPEED = 1.05D;

    private final EarthWolf pup;
    private EarthWolf playmate;
    private int playTicks;

    public EarthPupPlayGoal(EarthWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        ...
        List<EarthWolf> pups = this.pup.level().getEntitiesOfClass(
                EarthWolf.class,
                this.pup.getBoundingBox().inflate(PLAY_RADIUS),
                candidate -> candidate != this.pup
                        && candidate.isAlive()
                        && candidate.isBaby()
                        && this.pup.isEarthPackmate(candidate));
        if (pups.isEmpty()) {
            return false;
        }
