...
public final class LunarPupFollowAdultGoal extends Goal {
    ...
    private final LunarWolf pup;
    private LunarWolf adult;
    private int repathDelay;

    public LunarPupFollowAdultGoal(LunarWolf pup) {
        this.pup = pup;
    ...
    public boolean canUse() {
        if (!this.pup.isBaby()
                ...
                || this.pup.getBrain().hasMemoryValue(ModMemoryModuleTypes.LUNAR_PACK_THREAT.get())) {
            return false;
        }

        Optional<LunarWolf> rememberedAdult = this.pup.getBrain()
                .getMemory(ModMemoryModuleTypes.NEAREST_LUNAR_ADULT_PACKMATE.get());
        if (rememberedAdult.isEmpty() || !rememberedAdult.get().isAlive()) {
            return false;
        }

        LunarWolf candidate = rememberedAdult.get();
