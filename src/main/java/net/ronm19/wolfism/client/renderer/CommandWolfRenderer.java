...
/** Vanilla wolf geometry with Command Wolf's approved textures. */
public final class CommandWolfRenderer
        extends AgeableMobRenderer<CommandWolf, WolfRenderState, WolfModel> {

    private static final Identifier ADULT_TEXTURE =
            Identifier.fromNamespaceAndPath(
                    Wolfism.MOD_ID,
                    "textures/entity/wolf/command_wolf.png");

    private static final Identifier BABY_TEXTURE =
            Identifier.fromNamespaceAndPath(
                    Wolfism.MOD_ID,
                    "textures/entity/wolf/command_wolf_baby.png");

    public CommandWolfRenderer(EntityRendererProvider.Context context) {
        super(context, new AdultWolfModel(context.bakeLayer(ModelLayers.WOLF)), new BabyWolfModel
                (context.bakeLayer(ModelLayers.WOLF_BABY)), 0.5F);
    ...
    public void extractRenderState(
        ...
        state.isAngry = entity.isAngry()
                || entity.isCommandCombatVisualActive();
        ...
        state.texture =
                state.isBaby ? BABY_TEXTURE : ADULT_TEXTURE;
        state.wetShade =
                entity.getWetShade(partialTick);
        state.collarColor =
                entity.isTame() ? entity.getCollarColor() : null;
        state.bodyArmorItem =
